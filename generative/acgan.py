...
class ACGAN(object):
    ...
    def get_lipschitz(self):
        ...
        for i in range(self.class_num):
            for j in range(self.n_samples):
                ...
                L[i, j] = get_local_lipschitz(self.G, sample_z, sample_y, self.n_neighbors, 
                                                self.gpu_mode, self.z_dim, self.radius)
        ...
        for i in range(self.class_num):
            print("%d, %.2f, %.2f, %.2f, %.2f" % (i, 
                np.percentile(L[i,:], q=95), np.percentile(L[i,:], q=99),
    ...
    def reconstruct(self):
        ...
        samples = (samples + 1) / 2
        utils.save_images(samples[:100, :, :, :], [10, 10], 
        ...
        sample_y = torch.zeros(self.test_size, self.class_num).scatter_(1, labels_test, 1)
...
def get_local_lipschitz(G, sample_z, sample_y, n_neighbors, gpu_mode, z_dim=100, radius=1):
    ...
    u = torch.rand(n_neighbors) + 1e-24    # avoid underflow
